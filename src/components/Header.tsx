...
const Header = () => {
  ...
  const NavLinks = ({ mobile = false, onLinkClick }: { mobile?: boolean; onLinkClick?: () => void }) => (
    <>
      ...
      <Link 
        to="/location-voiture-coupe-monde-2026-maroc" 
        onClick={onLinkClick}
        className={`${mobile ? 'text-lg py-3 px-4 rounded-lg' : 'text-sm'} font-medium transition-colors hover:text-primary ${
          isActive('/location-voiture-coupe-monde-2026-maroc') ? 'text-foreground bg-accent' : 'text-muted-foreground'
        }`}
      >
        ⚽ Mondial 2026
      </Link>
  ...
  return (
    <header className="sticky top-0 z-50 w-full bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60 border-b-2 border-primary/20 shadow-sm">
